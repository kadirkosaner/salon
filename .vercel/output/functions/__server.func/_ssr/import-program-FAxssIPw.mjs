...
//#region node_modules/.nitro/vite/services/ssr/assets/import-program-FAxssIPw.js
var DOW_ENTRIES = [
	{
		key: "pazartesi",
		dow: 1
...
var previewProgramImport = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((text) => text).handler(previewProgramImport_createServerFn_handler, async ({ context, data: text }) => {
...
var commitProgramImport = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((d) => d).handler(commitProgramImport_createServerFn_handler, async ({ context, data }) => {
	const sql = await getSql();
	await ensureUserSeeded(sql, context.userId);
