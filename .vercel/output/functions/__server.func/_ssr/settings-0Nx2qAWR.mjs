...
import { s as parseOrThrow, t as authMiddleware } from "./validation-y1g7FGMs.mjs";
import { t as ensureUserProfile } from "./social-Co4clLoL.mjs";
import { getUserTimeZone, setUserTimeZone } from "./time-BN4ZvYw3.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/settings-0Nx2qAWR.js
...
}), d)).handler(updateSettings_createServerFn_handler, async ({ context, data }) => {
	const sql = await getSql();
	await ensureUserProfile(sql, context.userId);
...
var deleteMyAccount = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((d) => parseOrThrow(object({ confirm: literal("DELETE") }), d)).handler(deleteMyAccount_createServerFn_handler, async ({ context }) => {
