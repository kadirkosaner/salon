...
import { a as emitWorkoutCompleted, r as emitPersonalRecordIfAny } from "./activity-BAbxc4Wl.mjs";
import { s as isoDow, t as addDaysISO } from "./utils-DKNImH2A.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/workouts-CyRfYH4J.js
...
var clearFutureWorkouts = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator(noInput).handler(clearFutureWorkouts_createServerFn_handler, async ({ context }) => {
	const sql = await getSql();
...
var saveWorkoutToProgram = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator(v(object({ workoutId: positiveId }))).handler(saveWorkoutToProgram_createServerFn_handler, async ({ context, data }) => {
	...
	await sql`delete from program_exercises where program_day_id = ${programDayId}`;
	{
		...
		for (const ex of exercises) {
			...
			values.push(programDayId, ex.exercise_id, ex.detail, ex.target_sets, ex.target_rep_lo, ex.target_rep_hi, ex.rest_sec, ex.load_tag, ex.note, ex.sort);
		}
		await sql.query(`insert into program_exercises (
...
var ensureWorkoutHorizon = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator(v(object({
	untilDate: isoDate.optional(),
	daysAhead: number().int().min(1).max(120).optional()
}).default({}))).handler(ensureWorkoutHorizon_createServerFn_handler, async ({ context, data }) => {
	const sql = await getSql();
	await ensureUserSeeded(sql, context.userId);
	return ensureRollingHorizon(sql, context.userId, {
		coverUntil: data.untilDate,
...
var getPublicWorkout_createServerFn_handler = createServerRpc({
	id: "7d2d9941a491051447adf4078c81417e7de54a569e10302b6e90bc1a9e796d67",
	name: "getPublicWorkout",
	filename: "src/lib/server/workouts.ts"
}, (opts) => getPublicWorkout.__executeServer(opts));
var getPublicWorkout = createServerFn({ method: "GET" }).middleware([authMiddleware]).validator(v(positiveId)).handler(getPublicWorkout_createServerFn_handler, async ({ context, data: workoutId }) => {
	const sql = await getSql();
	const rows = await sql`
	...
	if (!isOwner) {
      ...
      `)[0]?.visibility || "public";
		if (vis === "private") throw new Error("This workout is private");
		if (vis === "followers") {
			if (!(await sql`
          select exists(
        ...
        `)[0]?.ok) throw new Error("This workout is private");
		}
	}
	...
	const mapped = exercises.map((ex) => {
		const sets = (setsByEx.get(ex.id) ?? []).map((s) => {
			...
			const reps = s.reps;
			if (s.completed && weight != null && reps != null && ex.unit === "kg") tonnage += weight * reps;
			return {
				set_index: s.set_index,
				weight,
				reps,
	...
	return {
		...
		program: program && (isOwner || program.is_public) ? program : null,
		exercises: mapped
	};
});
//#endregion
export { addWorkoutExercise_createServerFn_handler, clearFutureWorkouts_createServerFn_handler, createWorkout_createServerFn_handler, deleteWorkoutExercise_createServerFn_handler, deleteWorkout_createServerFn_handler, ensureWorkoutHorizon_createServerFn_handler, generateWorkouts_createServerFn_handler, getPublicWorkout_createServerFn_handler, getWorkoutByDate_createServerFn_handler, listWorkoutsInRange_createServerFn_handler, saveWorkoutToProgram_createServerFn_handler, skipWorkout_createServerFn_handler, swapWorkoutExercise_createServerFn_handler, updateWorkoutSet_createServerFn_handler, updateWorkout_createServerFn_handler };
