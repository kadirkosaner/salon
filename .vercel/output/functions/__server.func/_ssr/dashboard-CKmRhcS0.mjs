...
//#region node_modules/.nitro/vite/services/ssr/assets/dashboard-CKmRhcS0.js
...
var getDashboard = createServerFn({ method: "GET" }).middleware([authMiddleware]).handler(getDashboard_createServerFn_handler, async ({ context }) => {
	...
	const rangeStart = addDays(weekStart, -77);
...
var getExerciseProgress = createServerFn({ method: "GET" }).middleware([authMiddleware]).validator((name) => name).handler(getExerciseProgress_createServerFn_handler, async ({ context, data: name }) => {
