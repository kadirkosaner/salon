...
//#region node_modules/.nitro/vite/services/ssr/assets/import-program-0s4-CJcC.js
var DOW_ENTRIES = [
	{
		key: "pazartesi",
		dow: 1
...
var commitProgramImport = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator(v(object({
	...
	return withTransaction(async (sql) => {
      ...
      `)[0].id;
		const resolved = [];
		const customNames = /* @__PURE__ */ new Map();
		for (let di = 0; di < parsed.days.length; di++) {
			...
			for (let ei = 0; ei < day.exercises.length; ei++) {
				...
				if (!exerciseId) {
					...
					if (customNames.has(key)) exerciseId = customNames.get(key);
					else {
      ...
      `;
		...
		if (resolved.length > 0) {
			...
			for (const r of resolved) {
				...
				values.push(dayId, r.exerciseId, r.detail, r.sets, r.rep_lo, r.rep_hi, r.rest_sec, r.load_tag, r.note, r.sort);
			}
			if (placeholders.length > 0) await sql.query(`insert into program_exercises (
