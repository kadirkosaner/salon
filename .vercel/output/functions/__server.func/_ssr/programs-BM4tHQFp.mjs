...
//#region node_modules/.nitro/vite/services/ssr/assets/programs-BM4tHQFp.js
...
var listPrograms = createServerFn({ method: "GET" }).middleware([authMiddleware]).validator(noInput).handler(listPrograms_createServerFn_handler, async ({ context }) => {
...
var getActiveProgram = createServerFn({ method: "GET" }).middleware([authMiddleware]).validator(noInput).handler(getActiveProgram_createServerFn_handler, async ({ context }) => {
	const sql = await getSql();
	await ensureUserSeeded(sql, context.userId);
      ...
      `)[0].id;
		...
		];
		{
			...
			for (const d of days) {
				...
				values.push(programId, d.dow, d.name.trim() || `Gün ${d.dow}`, d.focus ?? null, sort);
				sort += 1;
			}
			if (placeholders.length > 0) await sql.query(`insert into program_days (program_id, dow, name, focus, sort)
